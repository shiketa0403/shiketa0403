...
| 変数 | 説明 | 例 |
|---|---|---|
| `{{案件名}}` | プログラム名 | airalo 紹介プログラム |
| `{{運営会社}}` | 会社名 | AIRGSM PTE. LTD. |
| `{{公式サイトURL}}` | 広告主のURL | https://www.airalo.com/ja |
| `{{公式サイト表示名}}` | リンクテキスト | airalo |
| `{{ジャンル}}` | 物販 or 登録 | 物販 |
| `{{報酬単価}}` | 報酬額 | 5% |
| `{{成果条件}}` | 成果発生の条件 | 申込完了 / 商品購入 |
| `{{承認基準}}` | 承認の基準 | 申込完了:申込確認<br>商品購入:入金確認 |
| `{{説明文}}` | AI生成の案件紹介文 | （後述のAI生成ルールで作成） |
