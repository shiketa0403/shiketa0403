...
- **`<p>`ブロック**で段落を分け、段落内は**1文ごとに空行**を入れる（句点ごとに改行＋空行）
- **装飾を必ず入れる**: 重要キーワードは `<span class="hutoaka">…</span>`、強調は `<strong>…</strong>`（報酬は生成器が `st-mymarker-s` で自動装飾）
- **分量は5〜6ブロック程度**（末尾の報酬文・提携ASP文＋CTAは生成器が自動付与するので、本文は4ブロック前後）
- **汎用的な文章で文字数を増やさない**。増やすのは**その広告固有の事実**（回線・容量・有効期間・特典・支払い方法・ターゲット等）のみ
