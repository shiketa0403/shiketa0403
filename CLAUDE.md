...
**agriwarriors.jp** に下書き投稿する仕組み。現状は東京都・神奈川県・千葉県・埼玉県・大阪府・兵庫県・京都府・福岡県に対応。
...
- 千葉: 千葉市（市専用ページがある前提）＋市川市・船橋市（`/farms/chiba` 単一ページから住所で分離）
...
- 福岡: `/farms/fukuoka` 単一ページ（`sb_slug` は空文字）から住所フィルタで分離。福岡市
