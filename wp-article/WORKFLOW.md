...
**ライトでも見出し粒度・装飾密度は通常と同じ（重要）**:
各H2（まとめ除く）は**H3 2〜3個に分割**し、H3ごとの文量・装飾の入れ方（prompts/07）は
通常記事と完全に同一にする。ライトで減らすのは**H2の本数＝扱うトピック数**であって、
1見出し配下の文章密度ではない。H3なしの長文H2・装飾のない本文の連続は機械監査で弾かれる。
